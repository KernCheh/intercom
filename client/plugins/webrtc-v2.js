(() => {
  ...
  const PHONE = window.PHONE = (config) => {
    ...
    function add_servers (servers) {
      if (servers.constructor === Array) { [].unshift.apply(rtcconfig.iceServers, servers) } else { rtcconfig.iceServers.unshift(servers) }
    }
    ...
    function get_conversation (number) {
      const talk = conversations[number] || ((number) => {
        const talk = {
          number,
          status: '',
    ...
    PHONE.bind = (type, el, fun) => {
      type.split(',').forEach((etype) => {
        ...
        } else if (el.attachEvent) { el.attachEvent('on' + etype, rapfun) } else { el['on' + etype] = rapfun }
      })
    }
    ...
    PHONE.unbind = (type, el, fun) => {
      if (el.removeEventListener) { el.removeEventListener(type, false) } else if (el.detachEvent) { el.detachEvent('on' + type, false) } else { el['on' + type] = null }
    }
    ...
    PHONE.dial = function (number, servers) {
      ...
      const talk = get_conversation(number)
      const pc = talk.pc
    ...
    PHONE.snap = function (message, number) {
      ...
      let pic = {}
      for (const number in conversations) { pic = conversations[number].snap() }
      return pic
    }
    ...
    PHONE.send = (message, number) => {
      ...
      for (const number in conversations) { conversations[number].send(message) }
    }
    ...
    PHONE.hangup = (number) => {
      ...
      for (const number in conversations) { conversations[number].hangup() }
    }
    ...
    PHONE.bind('unload,beforeunload', window, () => {
      ...
      for (const number in conversations) {
        const talk = conversations[number]
        const mynumber = config.number
        const packet = { hangup: true }
        const message = { packet, id: sessionid, number: mynumber }
        const client = new XMLHttpRequest()
        const url = 'https://pubsub.pubnub.com/publish/' +
                           pubkey + '/' +
                           subkey + '/0/' +
                           number + '/0/' +
    ...
    function snapshots_setup (stream) {
      const video = myvideo || document.createElement('video')
      const canvas = document.createElement('canvas')
      const context = canvas.getContext('2d')
      const snap = { width: 240, height: 180 }
      ...
      // Start Video Stream
      try { video.play() } catch (e) { debugcb(['video.play(FAIL)', e]) }
    ...
    function stopcamera () {
      if (!mystream) { return }
      for (const track of mystream.getTracks()) { track.stop() }
    }
    ...
    function transmit (phone, packet) {
      ...
      const message = { packet, id: sessionid, number }
      debugcb(['transmit(PASS)', message])
      pubnub.publish({ channel: phone, message })
    }
    ...
    function receive (message) {
      ...
      // Get Call Reference
      const talk = get_conversation(message.number)
    ...
    async function add_sdp_offer (message) {
      ...
      const talk = get_conversation(message.number)
      const pc = talk.pc
      const type = message.packet.type == 'offer' ? 'offer' : 'answer'
  ...
  function uuid (callback) {
    const u = 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g,
      function (c) {
  ...
  function socket (setup) {
    ...
    const subkey = setup.subscribe_key || setup.subkey || 'demo'
    ...
    function subscribe (setup) {
      const channel = setup.channel || 'a'
      const message = setup.message || (() => {})
      const connect = setup.connect || (() => {})
      const disconnect = setup.disconnect || (() => {})
      const reconnect = setup.reconnect || (() => {})
      let timetoken = setup.timetoken || '0'
      const timeout = setup.timeout || 300000
      const windy = setup.windowing || 10
      let restore = setup.restore
      let windowing = 10
      let connected = true
      let stop = false
      let url = ''
      ...
      function next (payload) {
        ...
        if (payload) {
          ...
          if (!restore) { timetoken = payload.t.t } else {
            timetoken = '1000'
            restore = false
          }
    ...
    function requester (setup) {
      ...
      let finished = false
      const timeout = setup.timeout || 5000
      const success = setup.success || function () {}
      const fail = setup.fail || function () {}
      ...
      xhr.onload = () => {
        ...
        try { result = JSON.parse(xhr.response) } catch (e) { fail(xhr) }

        if (result) { success(result) } else { fail(xhr) }
        result = null
      }
      ...
      return (setup) => {
        const url = setup.url || 'https://ps.pubnub.com/time/0'
        const headers = setup.headers || {}
        const method = setup.method || 'GET'
        const payload = setup.payload || null
        const params = setup.params || setup.data || {}
        const data = []

        // URL Parameters
        for (const param in params) { data.push([param, params[param]].join('=')) }
